...
void draw_minimap_grid(GameManager &game_manager) {
  ...
  for (const Tile &tile : game_manager.grid.tiles) {
    // if walkable, skip
    if (std::find(UNWALKABLE_TILES.begin(), UNWALKABLE_TILES.end(), tile.symbol) == UNWALKABLE_TILES.end()) {
      continue;
    }

    if (tile.symbol == Tile::Symbol::DOOR) {
      // draw filled rectangle
      sf::RectangleShape rectangle(sf::Vector2f(minimap_tile_size, minimap_tile_size));
      rectangle.setFillColor(sf::Color::White);
      rectangle.setPosition(tile.coords.x * minimap_tile_size, tile.coords.y * minimap_tile_size);
      game_manager.window.draw(rectangle);
    } else {
...
std::vector<std::optional<Raycast>> draw_walls_3d(GameManager &game_manager,
  ...
  for (int window_x = 0; window_x < render_width; window_x += ray_thickness) {
    ...
    float
        ray_angle_deg =
        game_manager.player.dir_deg - game_manager.camera.fov_horizontal_deg / 2
            + rotation_step_deg * (float) window_x;
...
void draw_hud_bar(GameManager &game_manager, sf::Sprite &hud_sprite) {
  float
      ...
      (float) hud_sprite.getTexture()->getSize().x / (float) hud_sprite.getTexture()->getSize().y;
...
void draw_pickups(GameManager &game_manager, std::vector<std::optional<Raycast>> &walls_raycast) {
  int render_height = (int) game_manager.window.getSize().y - game_manager.hud.bar_height;
  auto sorted_pickups_info =
      compute_sort_desc_pickup_distance_to_player_vec(game_manager.player.pos, game_manager.pickups);

  // set pickup sprites position and scale
  for (auto &sorted_pickup_info : sorted_pickups_info) {
    sf::Sprite &pickup_sprite = sorted_pickup_info.pickup_ref.get().get_sprite();
    float column_height =
        std::fmin((float) render_height * (game_manager.grid.tile_size / sorted_pickup_info.distance), 10000);
    float scale_factor = column_height / 64; // TODO: get frame width instead of magic 64
    pickup_sprite.setScale(scale_factor, scale_factor);

    // compute angle difference between player and pickup
    float angle_diff_deg = get_angle_between_vectors_deg(game_manager.player.get_dir_vector(),
                                                         get_difference_vector(game_manager.player.pos,
                                                                               sorted_pickup_info.pickup_ref.get().get_pos()));
    ...
    float window_x = window_center_x + difference_x - (pickup_sprite.getGlobalBounds().width) / 2;
  ...
  for (auto &sorted_pickup_info : sorted_pickups_info) {
    if (sorted_pickup_info.pickup_ref.get().is_disabled) {
      continue;
    }

    sf::Sprite &pickup_sprite = sorted_pickup_info.pickup_ref.get().get_sprite();
    int sprite_start_x = pickup_sprite.getPosition().x;
    int sprite_end_x = pickup_sprite.getPosition().x + pickup_sprite.getGlobalBounds().width;
    ...
    for (int window_x = loop_start_x; window_x < loop_end_x; window_x++) {
      ...
      float sprite_x_percentage = (float) sprite_x / (float) pickup_sprite.getGlobalBounds().width;
      ...
      if (wall_raycast.has_value()) {
        ...
        float pickup_distance = sorted_pickup_info.distance;

        if (pickup_distance > wall_distance) {
          continue;
        }
      }

      // Draw the sprite only for this x column
      sf::IntRect texture_rect = pickup_sprite.getTextureRect();
      texture_rect.left += texture_x;
      texture_rect.width = 1;

      sf::Sprite column_sprite(*pickup_sprite.getTexture(), texture_rect);
      column_sprite.setPosition(window_x, pickup_sprite.getPosition().y);
      column_sprite.setScale(pickup_sprite.getScale());
...
*/
  ...
  // 3d walls
  std::vector<std::optional<Raycast>>
      walls_raycast = draw_walls_3d(game_manager, wall_sprite, wall_shadow_sprite, door_sprite, door_shadow_sprite);

  draw_pickups(game_manager, walls_raycast);
